...
import ch.viseca.flink.connectors.jdbc.sources.BatchStructuredDataJdbcSourceConnector
import org.apache.flink.api.common.state.ListStateDescriptor
import org.apache.flink.api.common.typeinfo.{TypeHint, TypeInformation}

class MockBatchStructuredDataJdbcSourceConnector[Out <: Product : TypeInformation]
(
  extractEv: (ResultSet, Long, Long) => Out
  , extractBk: (ResultSet) => Option[Long]
  , extractBt: (ResultSet) => Long
  , extractMk: (ResultSet) => Long
  , extractEt: (ResultSet) => Long
  , dataSeq: Seq[Out]*
)
  extends BatchStructuredDataJdbcSourceConnector[Out, Long, Long](0) {
  ...
  override def extractBatchKey(rs: ResultSet): Option[Long] = extractBk(rs)

  override def extractBatchTimestamp(rs: ResultSet): Long = extractBt(rs)
