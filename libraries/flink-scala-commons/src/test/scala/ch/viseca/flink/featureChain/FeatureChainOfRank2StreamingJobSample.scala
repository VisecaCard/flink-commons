...
object FeatureChainOfRank2StreamingJobSample {
  ...
  def main(args: Array[String]): Unit = {
    ...
    implicit var env: JobEnv = StreamExecutionEnvironment.getExecutionEnvironment
    ...
    val collectedPersonFeaturesSink = new JobSink[EnrichedPerson]() {
      override def bind(stream: DataStream[EnrichedPerson])(implicit env: JobEnv) = {
        stream.print(); BoundSink(this, stream)
      }
    }

    val job = new FeatureChainOfRank2StreamingJob(persons, countries)

    job.bind

    collectedPersonFeaturesSink.bind(job.collectedPersonFeatures)
  ...
  class FeatureChainOfRank2StreamingJob
  (
    val persons: DataStream[Person],
    val countries: DataStream[Country]
  )
  (implicit env: JobEnv) extends StreamingJob {
  ...
  (
    ...
    addressCountryInput: DataStream[_],
    ...
    addressCountryInput.name("addressCountryInput")
