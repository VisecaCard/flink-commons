...
class AssignUUIDFunctionTest extends WordSpec with Matchers {
  ...
  trait Fixture {
    ...
    val stream =
      env.fromCollection(getEvents)
        .map(new AssignUUIDFunction[testEvent])

    stream.addSinkOperator(new CollectorSinkOperator[EventWithId[UUID, testEvent]]("outputCollection"))

    val elems = DataStreamUtils.collect(stream.javaStream).asScala.toList
  }

  "AssignUUIDFunction" when {
    ...
    "mapping a multiple event stream" should {
